...
# 물리: physics_v2.eff_poa (FABLE_REVIEW.md §C) · 데이터: bipv_ai_master_data_v16.csv
# ==============================================================================
import json
import numpy as np
import pandas as pd
...
df = pd.read_csv("bipv_ai_master_data_v16.csv")
dd = df[df[EL] > 0].copy()
...
e_ai = energy(dd.target_angle_v16.values)
...
R["ai_vs_fixed_pct"] = {t: round((e_ai / fixed[t] - 1) * 100, 2)
                        for t in (15, 30, 37, 45, best_fix, 60, 68, 75, 90)}
...
for key, g in (("cloudy_cc_ge_0.7", dd[dd.cloud_cover >= 0.7]),
    ...
    R[f"ai_vs_bestfix_{key}_pct"] = round(
        (energy(g.target_angle_v16.values, g) / energy(np.full(len(g), float(best_fix)), g) - 1) * 100, 2)

# [6] 환기 시나리오 (가정: 여름 6~8월 13~17시 블레이드 강제 ≤30° 개방)
vent = (dd.month.isin([6, 7, 8]) & dd.hour.between(13, 17)).values
ai_vent = dd.target_angle_v16.values.copy(); ai_vent[vent] = 30.0
neglect = dd.target_angle_v16.values.copy(); neglect[dd.month.isin([6, 7, 8]).values] = 30.0
