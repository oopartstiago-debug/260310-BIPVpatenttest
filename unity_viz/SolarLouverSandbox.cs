...
public class SolarLouverSandbox : MonoBehaviour {
    ...
    [Range(0f, 90f)]     public float baseline = 60f;
    ...
    void OnGUI() {
        ...
        float W = 360;
        GUILayout.BeginArea(new Rect(Screen.width - W - 18, 18, W, 320), GUI.skin.box);
        GUILayout.Label("샌드박스 — 우리 현장이면? (LouverPhysics)", st);
        GUILayout.Label($"알베도 {albedo:0.00}", st);   albedo   = GUILayout.HorizontalSlider(albedo, 0.05f, 0.4f);
        GUILayout.Label($"현 chord {chord:0.0}mm", st);  chord    = GUILayout.HorizontalSlider(chord, 50f, 130f);
        GUILayout.Label($"피치 pitch {pitch:0.0}mm", st); pitch   = GUILayout.HorizontalSlider(pitch, 60f, 130f);
        GUILayout.Label($"비교 고정각 {baseline:0}°", st); baseline = GUILayout.HorizontalSlider(baseline, 0f, 90f);
        GUILayout.Space(8);
        GUILayout.Label($"현/피치 비율 = {chord / pitch:0.00}", st);
        GUILayout.Label($"oracle 최적각(정오) = {oracleNoon:0}°", st);
        GUILayout.Label($"하루 최고고정각 = {bestFixed:0}°", st);
        GUILayout.Label($"AI 이득  vs 고정{baseline:0}° = +{dayGainVsBase:0.0}%", st);
        GUILayout.Label($"AI 이득  vs 최고고정 = +{dayGainVsBest:0.0}%", st);
        GUILayout.Space(6);
